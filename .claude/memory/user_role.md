...
- Vivek prefers to get multi-step execution done autonomously once direction is agreed. Expects proactive swarm-mode work, not question-by-question.
- Shubham communicates via GitHub issues in short descriptive text — take literal reading of the issue, then apply YAGNI when interpreting.
- Session work is typically shipped to `feature/foundation` and landed to `main` in bulk (no per-issue PR).

When summarising: tight tables/bullets, no emojis, file:line references for evidence.
