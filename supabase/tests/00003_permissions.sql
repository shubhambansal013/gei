...
BEGIN
    ...
    -- 2. Test can_user with STORE_MANAGER access on site
    INSERT INTO site_user_access (user_id, site_id, role_id)
    VALUES (u1_id, s_id, 'STORE_MANAGER');

    -- 3. Test can_user with SUPER_ADMIN global role
    INSERT INTO profiles (id, full_name, role_id, is_active)
    VALUES (u2_id, 'Super Admin ' || u_code, 'SUPER_ADMIN', true);
END $$;

SELECT is(
    can_user((SELECT id FROM profiles WHERE full_name LIKE 'Test User %' ORDER BY created_at DESC LIMIT 1), (SELECT id FROM sites WHERE name LIKE 'Perm Test Site %' ORDER BY created_at DESC LIMIT 1), 'INVENTORY', 'CREATE'),
    true,
    'STORE_MANAGER on site should have INVENTORY.CREATE permission'
);

SELECT is(
    can_user((SELECT id FROM profiles WHERE full_name LIKE 'Test User %' ORDER BY created_at DESC LIMIT 1), (SELECT id FROM sites WHERE name LIKE 'Perm Test Site %' ORDER BY created_at DESC LIMIT 1), 'MASTERS', 'DELETE'),
    false,
    'Regular STORE_MANAGER should not have MASTERS.DELETE permission'
);

SELECT is(
    can_user((SELECT id FROM profiles WHERE full_name LIKE 'Super Admin %' ORDER BY created_at DESC LIMIT 1), (SELECT id FROM sites WHERE name LIKE 'Perm Test Site %' ORDER BY created_at DESC LIMIT 1), 'MASTERS', 'DELETE'),
    true,
    'SUPER_ADMIN should have global permission for MASTERS.DELETE'
);

-- Test inactive user
UPDATE profiles SET is_active = false WHERE full_name LIKE 'Super Admin %';
